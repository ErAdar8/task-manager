...
export default function SubtopicDetailPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [courseRes, subtopicRes, learningsRes] = await Promise.all([
      fetch(`/api/courses/${courseId}`),
      fetch(`/api/subtopics/${subtopicId}`),
      fetch(`/api/learnings?subtopicId=${encodeURIComponent(subtopicId)}`),
    ]);
    const [cJson, stJson, lJson] = await Promise.all([
      courseRes.json() as Promise<{ success: boolean; data?: Course }>,
      subtopicRes.json() as Promise<{ success: boolean; data?: Subtopic }>,
      learningsRes.json() as Promise<{ success: boolean; data?: StandaloneLearning[] }>,
    ]);
    if (cJson.success && cJson.data) setCourse(cJson.data);
    if (stJson.success && stJson.data) setSubtopic(stJson.data);
    if (lJson.success && lJson.data) setItems(lJson.data);
    setLoading(false);
  }, [courseId, subtopicId]);
  ...
  const createCard = async () => {
    if (!subtopicId || !courseId || !createContent.trim()) return;
    setSaving(true);
    try {
  ...
  return (
    <main className="flex-1 overflow-auto bg-slate-950 text-slate-100 p-6 md:p-8">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-semibold">{subtopic.title}</h1>
          {subtopic.description && (
            <p className="text-slate-400 text-sm mt-1">{subtopic.description}</p>
          )}
        </div>
          ...
          <Card className="border-violet-800/50 bg-violet-950/20">
            <CardContent className="p-4 space-y-3">
              ...
              <Textarea
                placeholder={
                  createType === "image"
                    ? "Paste image description or notes..."
                    : createType === "flow"
